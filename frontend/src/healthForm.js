...
function HealthForm() {
  ...
  const ValSchema = Yup.object().shape({
    studentId: Yup.string().matches(/^\d{8}[A-Za-z]$/, {
      message: "Example: 12345678d",
    }),
    ...
    isSuspected: Yup.string().required("Required"),
    isConfirmed: Yup.string().required("Required"),
    bodyTemperature: Yup.string().matches(/^[34]\d\.\d$/, {
      message: "Example: 36.0",
    }),
  });
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      console.log("Submit");
      axios
        .post("http://backend-dev22222222.us-east-1.elasticbeanstalk.com/", values)
      ...
      <form onSubmit={formik.handleSubmit}>
        ...
        >
          <InputLabel id="country-select">
            Country/Region you are residing in *
          </InputLabel>
          <Select
            name="country"
          ...
          >
            {Countries.map((item) => {
              return (
                <MenuItem key={item.abbr} value={item.name}>
                  {item.name}
                </MenuItem>
              );
            })}
        ...
        <FormControl className={classes.formControl}>
          <TextField
            ...
            error={
              !!(
                formik.errors.bodyTemperature && formik.touched.bodyTemperature
              )
            }
        ...
        >
          <FormLabel className={classes.lineHeight12} component="legend">
            Have you developed symptoms like fever and cough in the past 14
            days?
          </FormLabel>
          <FormHelperText>{formik.errors.isSuspected}</FormHelperText>
          <RadioGroup
            name="isSuspected"
            value={formik.values.isSuspected}
            onChange={formik.handleChange}
          >
            <Box display="flex">
